// This time we learn about two useful methods of Number objects: toString() and toLocaleString(). the basic usage of the two methods is to convert a number 
...
function colorOf(r,g,b){
    let rToString = r.toString(16);
    if (rToString[0] === '0') {
        rToString = '0' + rToString
    } else if (rToString.length === 1) {
        rToString = '0' + rToString;
    };
    
    let gToString = g.toString(16);
    if (gToString[0] === '0') {
        gToString = '0' + gToString
    } else if (gToString.length === 1) {
        gToString = '0' + gToString;
    };

    let bToString = b.toString(16);
    if (bToString[0] === '0') {
        bToString = '0' + bToString
    } else if (bToString.length === 1) {
        bToString = '0' + bToString;
    };

    return `#${rToString}${gToString}${bToString}`;
};  
